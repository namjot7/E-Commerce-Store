...
const Product = ({ _id, name, description, price, picture }) => {
    ...
    return (
        <div className="products w-[240px] m-4 px-3">
            <div className="bg-blue-100 flex items-center justify-center p-4  rounded-lg">
                <img className="w-[220px] h-[200px] object-contain" src={picture} alt={picture} />
            </div>
            <div className='overflow-hidden h-[57px] mt-3'>
                <p className="text-xl ">{name}</p>
                <p className="text-sm leading-5 text-gray-400">{description}</p>
            </div>
            <div className="text-lg flex justify-between items-center">
                <span>${price}</span>
                <button onClick={e => addProduct()} className="bg-green-500 px-3 py-1 rounded-lg hover:bg-green-600">
