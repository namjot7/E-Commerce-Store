...
const Footer = () => {
    ...
    return (
        <footer className='text-gray-400 text-sm sticky bottom-0 flex justify-center gap-6 py-3 bg-slate-900 border-t border-gray-900'>
            ...
            <Link href="/cart" className={`${path === '/cart' ? 'text-emerald-300  font-bold' : ''} relative flex flex-col justify-center items-center`}>
                ...
                <span className='absolute -top-1.5 -right-3 rounded-full text-emerald-300 font-bold'>{selectedProducts.length} </span>
            </Link>
        </footer >
    )
}
